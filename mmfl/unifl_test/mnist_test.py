...
class CombinedModel(nn.Module):
    def __init__(self, input_dims, output_dims, kernel_sizes, num_classes):
        ...
        self.attention = AttentionLayer(hidden_dim=7)
        # Concatenation doubles the feature size, so adjust the input dimension for the classifier
        self.classifier = Classifier(2 * 64 * 7 * 7, num_classes)  # Include 7 * 7 for spatial dimensions
...
# Initialize model
input_dims = [1, 8, 16, 32]  # MNIST has 1 channel (grayscale)
